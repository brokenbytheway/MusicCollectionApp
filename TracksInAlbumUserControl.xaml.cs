...
{
    public partial class TracksInAlbumUserControl : UserControl
    {
        ...
        private ObservableCollection<TrackModel> _albumTracks;
        public ObservableCollection<TrackModel> AlbumTracks => _albumTracks;
        public AlbumModel Album => album;
        public event Action BackButtonClick;

        public TracksInAlbumUserControl(AlbumsUserControl parent, AlbumModel albumToOpen)
        {
            InitializeComponent();
            parentControl = parent;
            album = albumToOpen;
            _musicPlayer = MusicPlayerService.Instance;
            _albumTracks = new ObservableCollection<TrackModel>();
            DataContext = this;
        ...
        private void LoadAlbumTracks()
        {
            ...
            MySqlCommand command = new MySqlCommand("SELECT track_id, track_title, album_id, number_in_album, genre_id, track_release_year, path_to_track_mp3_file, path_to_track_cover, is_single FROM TRACKS WHERE album_id = @album_id ORDER BY number_in_album ASC", connection);
            ...
            using (MySqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    _albumTracks.Add(new TrackModel(trackId, title, albumId, numberInAlbum, genre, releaseYear, coverPath, mp3Path, isSingle, artists));
